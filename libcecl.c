                                   ...
                                   size_t *param_value_size_ret)
{
  ...
  if (err == CL_INVALID_DEVICE)
    ...
    fprintf(stderr,
            "param_name is not one of the supported values or "
 ...
 * No return if fails.
 */
static cl_int cecl_get_kernel_info(cl_kernel kernel, cl_kernel_info param_name,
                                   size_t param_value_size, void *param_value,
                                   size_t *param_value_size_ret)
{
  ...
  if (err == CL_INVALID_VALUE)
    fprintf(stderr,
            "param_name is not valid, or size in bytes specified "
...
static void cec_wait_for_event(cl_event *event)
{
  ...
  {
    fprintf(stderr,
            "events specified in event_list do not belong to the "
            "same context.\n");
  }
  else if (err == CL_INVALID_EVENT)
  {
    fprintf(stderr,
            "event objects specified in event_list are not valid "
            "event objects.\n");
  }
  exit(E_CL_FAILURE);
}
                            ...
                            const cl_event *event_wait_list, cl_event *event)
{
  ...
  if (err == CL_INVALID_PROGRAM_EXECUTABLE)
    fprintf(stderr,
            "there is no successfully built program "
    ...
    fprintf(stderr,
            "the kernel argument values have not been "
            "specified.\n");
  else if (err == CL_INVALID_WORK_DIMENSION)
    fprintf(stderr,
            "work_dim is not a valid value "
            "(i.e. a value between 1 and 3).\n");
  else if (err == CL_INVALID_WORK_GROUP_SIZE)
    fprintf(stderr,
            "local_work_size is specified and number "
    ...
    fprintf(stderr,
            "local_work_size is specified and the "
    ...
    fprintf(stderr,
            "the number of work-items specified in "
    ...
    fprintf(stderr,
            "there is a failure to queue the "
    ...
    fprintf(stderr,
            "there is a failure to allocate "
...
#pragma GCC diagnostic pop
  ...
  if (err == CL_INVALID_PROGRAM_EXECUTABLE)
    fprintf(stderr,
            "there is no successfully built program executable "
    ...
    fprintf(stderr,
            "context associated with command_queue and kernel is "
...
static cl_platform_id cecGetForcedPlatformIdOrDie()
{
  ...
  {
    fprintf(stderr,
            "[CECL] Required environment variable "
  ...
  {
    size_t buffer_size;
    local_err = clGetPlatformInfo(platforms[i], CL_PLATFORM_NAME, 0, NULL,
                                  &buffer_size);
    if (local_err != CL_SUCCESS)
    {
      fprintf(stderr,
              "Cannot get the size of the CL_PLATFORM_NAME "
              "parameter\n");
      exit(E_CL_FAILURE);
    }
...
static cl_device_id cecGetForcedDeviceIdOrDie()
{
  ...
  {
    fprintf(stderr,
            "[CECL] Required environment variable LIBCECL_DEVICE "
            "not set!\n");
    exit(E_CL_FAILURE);
  }
  ...
  for (i = 0; i < device_count; ++i)
  {
    ...
    size_t n = buffer_size < 512 ? buffer_size : 512;

    char *trimmed_buffer = buffer;
    while ((*trimmed_buffer == ' ') && (n > 0))
    {
      trimmed_buffer++;
      n--;
    }

    if ((n > 0) && (!strncmp(trimmed_buffer, target_device, n)))
    {
...
cl_context CECL_CREATE_CONTEXT(
    cl_context_properties *properties_unused, cl_uint num_devices,
    const cl_device_id *unused_devices,
    void *unused_pfn_notify(const char *errinfo, const void *private_info,
    ...
    void *user_data, cl_int *errcode_ret)
{
  ...
  {
    fprintf(stderr,
            "[CECL] libcecl only supports OpenCL contexts for 1 "
            "device!");
    exit(E_CL_FAILURE);
  }
  ...
  return clCreateContext(NULL, 1, &device_id, NULL, user_data, errcode_ret);
}

cl_context CECL_CREATE_CONTEXT_FROM_TYPE(
    cl_context_properties *properties_unused, cl_device_type device_type_unused,
...
#pragma GCC diagnostic pop
  if (local_err == CL_SUCCESS)
  {
    ...
    cecl_get_device_info(device_id, CL_DEVICE_TYPE, sizeof(devtype), &devtype,
                         NULL);
    cecl_get_device_info(device_id, CL_DEVICE_NAME, sizeof(devname), devname,
                         NULL);
  ...
  if (local_err == CL_INVALID_CONTEXT)
    ...
    fprintf(stderr,
            "device is not a valid device or is not associated "
            "with context.\n");
    ...
    fprintf(stderr,
            "values specified in properties are valid but are not "
...
cl_program CECL_PROGRAM_WITH_SOURCE(cl_context context, cl_uint count,
                                    ...
                                    cl_int *err)
{
  ...
  cl_uint i;
  cl_program p =
      clCreateProgramWithSource(context, count, strings, lengths, &local_err);
...
cl_program CECL_PROGRAM_WITH_BINARY(cl_context context, cl_uint num_devices,
                                    ...
                                    const size_t *lengths,const char **strings,
                                    cl_int *err)
{
  FILE *fp;
      size_t buffer_size;
    cecl_get_device_info(device_list[0], CL_DEVICE_NAME, 0, NULL, &buffer_size);
...
cl_int cecl_program(cl_program program, cl_uint num_devices,
                    ...
                    void *user_data, const char *program_name)
{
  ...
  if (err == CL_INVALID_PROGRAM)
    ...
    fprintf(stderr,
            "device_list is NULL and num_devices is greater than "
    ...
    fprintf(stderr,
            "program is created with clCreateProgramWithSource and "
...
cl_kernel cecl_kernel(cl_program program, const char *program_name,
                      const char *kernel_name, cl_int *err)
{
  cl_int local_err;
  ...
  if (local_err == CL_INVALID_PROGRAM)
    ...
    fprintf(stderr,
            "there is no successfully built executable for "
            "program.\n");
    ...
    fprintf(stderr,
            "the function definition for __kernel function given "
                                      ...
                                      cl_uint *num_kernels_ret)
{
  ...
  if (local_err == CL_INVALID_PROGRAM)
    ...
    fprintf(stderr,
            "kernels is not NULL and num_kernels is less than the "
...
cl_int cecl_write_buffer(cl_command_queue command_queue, cl_mem buffer,
                         const char *buffer_name, size_t offset, size_t cb,
                         ...
                         const cl_event *event_wait_list, cl_event *event)
{
  ...
  if (err == CL_INVALID_COMMAND_QUEUE)
    ...
    fprintf(stderr,
            "the context associated with command_queue and buffer "
    ...
    fprintf(stderr,
            "the region being read specified by (offset, cb) is "
    ...
    fprintf(stderr,
            "there is a failure to allocate memory for data store "
...
cl_int cecl_set_kernel_arg(cl_kernel kernel, cl_uint arg_index, size_t arg_size,
                           const void *arg_value, const char *arg_name)
{
...
cl_mem cecl_buffer(cl_context context, cl_mem_flags flags, size_t size,
                   void *host_ptr, cl_int *err, const char *host_ptr_name,
                   const char *flags_name)
{
  ...
  if (local_err == CL_INVALID_CONTEXT)
    ...
    fprintf(stderr,
            "values specified in flags are not valid as defined "
            "in the table above.\n");
  else if (local_err == CL_INVALID_BUFFER_SIZE)
    fprintf(stderr,
            "size is 0. Implementations may return "
    ...
    fprintf(stderr,
            "there is a failure to allocate memory for buffer "
            "object.\n");
...
cl_int cecl_read_buffer(cl_command_queue command_queue, cl_mem buffer,
                        const char *buffer_name, size_t offset, size_t cb,
                        ...
                        const cl_event *event_wait_list, cl_event *event)
{
  ...
  cl_int err = clEnqueueReadBuffer(
      command_queue, buffer, true, /* blocking read */
      offset, cb, ptr, num_events_in_wait_list, event_wait_list, event);
  ...
  if (err == CL_INVALID_COMMAND_QUEUE)
    ...
    fprintf(stderr,
            "the region being read specified by (offset, size) is "
    ...
    fprintf(stderr,
            "buffer is a sub-buffer object and offset specified "
    ...
    fprintf(stderr,
            "the read and write operations are blocking and the "
    ...
    fprintf(stderr,
            "clEnqueueReadBuffer is called on buffer which has "
...
void *cecl_map_buffer(cl_command_queue command_queue, cl_mem buffer,
                      const char *buffer_name, cl_map_flags map_flags,
                      ...
                      cl_int *errcode_ret)
{
  ...
  if (!errcode_ret)
  { /* in case errcode_ret points to NULL */
    errcode_ret = &local_err;
  }

  void *ret = clEnqueueMapBuffer(command_queue, buffer, true, /* blocking map */
  ...
  if (*errcode_ret == CL_INVALID_COMMAND_QUEUE)
    ...
    fprintf(stderr,
            "the context associated with command_queue, src_image "
    ...
    fprintf(stderr,
            "region being mapped given by (offset, cb) is out of "
    ...
    fprintf(stderr,
            "there is a failure to map the requested region into "
