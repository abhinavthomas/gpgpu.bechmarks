...
class DumpLogProtoToFileObserver(BenchmarkRunObserver):
  ...
  def OnBenchmarkRun(self, log: gpgpu_pb2.BenchmarkRun) -> bool:
    ...
    d = d.replace(microsecond=int(d.microsecond / 1000) * 1000 + self._cyclic_counter) 
...
def MakeEnv(
  ...
  with fs.chdir(make_dir):
    with tempfile.TemporaryDirectory(prefix='libcecl_header_') as d:
      d = pathlib.Path(d)
      env = os.environ.copy()
      cflags, ldflags = libcecl_run.compile_flags(opencl_headers=opencl_headers)
...
class _BenchmarkSuite(object):
  ...
  def _ExecToLogFile(
    ...
    with self.RunEnv(executable.parent) as os_env:
      ...
      libcecl_log = libcecl_run.execute(command, self.env, os_env, record_outputs=_RECORD_OUTPUTS)
      log = gpgpu_pb2.BenchmarkRun(
        hostname = _HOSTNAME,
...
def main(bmark_suites, gpgpu_envs, repeats, logdir):
  ...
  for benchmark_suite_class in ResolveBenchmarkSuiteClassesFromNames(bmark_suites):
    with benchmark_suite_class() as benchmark_suite:
      for env in envs:
        logging.info('Building and running %s on %s', benchmark_suite.name, env.name)
...
if __name__ == "__main__":
  env_names = ['CPU|Intel(R) CPU Runtime for OpenCL(TM) Applications|Intel(R) Xeon(R) CPU E5-2650 v2 @ 2.60GHz|18.1.0.0920|2.1']
